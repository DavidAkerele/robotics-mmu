...
**Observations:**  
The robot moved in a roughly square pattern. Since this is open-loop control using `time.sleep()`, the square was not perfectly closed due to minor wheel slip and simulation drift. However, the logic for sequential linear and angular velocity commands was correctly implemented.
...
Added `message_generation` and `message_runtime` dependencies.

**Question 3-5: Develop a client and server that utilize the .srv message.**  
Implemented `davidakerele_server.py` and `davidakerele_client.py`.

**Observations:**  
The service-oriented architecture allowed for remote procedure calls where the robot's high-level task (moving in squares) was triggered by a client request. The server successfully handled the `sideLength` and `repetitions` parameters, returning `True` upon completion. This modularity is a key feature of complex robotic systems.
...
The Robotics Science and Systems (RSS) unit has been a transformative experience in my academic journey. Prior to this module, my understanding of robotics was largely theoretical. Through the practical assignments (PA 1-6), I gained hands-on experience with the Robot Operating System (ROS), which is the industry standard for robotic development. 

Learning to implement publishers, subscribers, and services (PA 3 and PA 6) taught me the importance of asynchronous communication and modular design. Moving from open-loop control (PA 2) to closed-loop feedback systems (PA 4) was a key "lightbulb moment," highlighting why sensors like Odometry are indispensable. The final technical report (PA 7-8) allowed me to bridge the gap between practical coding and cutting-edge research, specifically in the field of SLAM. This unit has not only equipped me with technical skills but also with a critical mindset for evaluating robotic solutions in real-world, unpredictable environments.
